...
function App() {
  ...
  async function loadPreview(overrideTimeH?:string, overridePrn?:string, forceEpoch=false) { await runOperation('preview', 'Loading map points…', async () => { const requestedTime = overrideTimeH ?? timeH; const requestedPrn = overridePrn ?? prn; const requestedMode = forceEpoch ? 'epoch' : mapMode; const q = new URLSearchParams({ tolerance_seconds:String(tol), max_points:String(maxPoints) }); if (requestedPrn) q.set('prn', requestedPrn); if (requestedMode === 'epoch' && requestedTime !== '') q.set('time_h', requestedTime); if (requestedMode === 'window') { if (startTimeH !== '') q.set('start_time_h', startTimeH); if (endTimeH !== '') q.set('end_time_h', endTimeH); } const r = await fetch(`${API}/api/preview/points?${q}`); if (!r.ok) throw new Error(await r.text()); const j = await r.json(); const visible = (j.points || []).filter(inBounds); setPoints(visible); setStationMarkers(j.station_markers || []); setRasterAvailable(Boolean(j.raster_available)); setLimitReached(Boolean(j.limit_reached)); const actual = j.actual_time_h ?? j.requested_time_h ?? (requestedMode === 'epoch' ? requestedTime : 'n/a'); if (requestedMode === 'epoch' && j.actual_time_h != null) setTimeH(String(Number(j.actual_time_h).toFixed(5))); const msg = `Map ${j.mode_used}; PRN ${requestedPrn || 'All'}; epoch ${actual}; ${visible.length}/${j.total_matching_before_limit} points; limit ${j.limit_reached ? 'reached' : 'not reached'}`; setPreviewMeta(msg); setStatus({stage:'done', current:1, total:1, message:msg}); }); }
