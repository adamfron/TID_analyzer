...
type MapLayers = { borders:boolean; grid:boolean; ipp:boolean; stations:boolean; dtec:boolean };
...
function App() {
  ...
  const [layers, setLayers] = useState<MapLayers>({borders:true, grid:true, ipp:true, stations:true, dtec:false});
...
function Explorer(p:any) {
  ...
  return <div className="summary">
    ...
    <fieldset className="layers"><legend>Layers</legend>{[['borders','Show country borders'],['grid','Show grid'],['ipp','Show IPP points'],['stations','Show station markers']].map(([key,label])=><label key={key}><input type="checkbox" checked={p.layers[key]} onChange={(e:any)=>p.setLayers((x:MapLayers)=>({...x,[key]:e.target.checked}))}/>{label}</label>)}<label title={p.rasterAvailable ? 'Show interpolated dTEC raster.' : 'Interpolated dTEC raster is not loaded yet.'}><input type="checkbox" checked={p.layers.dtec && p.rasterAvailable} disabled={!p.rasterAvailable} onChange={(e:any)=>p.setLayers((x:MapLayers)=>({...x,dtec:e.target.checked}))}/>Show interpolated dTEC map</label>{!p.rasterAvailable && <p className="mini">Interpolated dTEC map TODO; toggle enables after raster data is available.</p>}</fieldset>
