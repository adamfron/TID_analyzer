...
type MapDisplaySettings = { ippMarkerSize:number; stationMarkerSize:number; stationLabelSize:number; rayWidth:number; rayOpacity:number };
const DEFAULT_MAP_DISPLAY: MapDisplaySettings = { ippMarkerSize:4, stationMarkerSize:7, stationLabelSize:10, rayWidth:0.8, rayOpacity:30 };
function loadMapDisplaySettings():MapDisplaySettings { try { return {...DEFAULT_MAP_DISPLAY, ...JSON.parse(window.localStorage.getItem('tid.map.display') || '{}')}; } catch { return DEFAULT_MAP_DISPLAY; } }
...
type Arc = { prn:string; arc_index:number; total_epoch_count?:number; usable_epoch_count?:number; low_coverage_epoch_count?:number; first_usable_time_h?:number|null; last_usable_time_h?:number|null; no_usable_epoch_reason?:string|null; start_time_h:number; end_time_h:number; duration_min:number; row_count:number; station_count:number; epoch_count:number; max_station_count:number; median_station_count:number; eligible_for_interpolation:boolean; ineligibility_reasons:string[]; interpolation_status:string; generated_map_count:number; failed_map_count:number };
...
function App() {
  ...
  useEffect(() => { window.localStorage.setItem('tid.map.display', JSON.stringify(mapDisplay)); }, [mapDisplay]);
  useEffect(() => {
...
function Visibility({arcs,load,choose,loading,summary,progress,startInterpolation,startArcInterpolation,cancelInterpolation,selectedArcKey}:any) {
...
function exportActiveMap(prn:string, timeH:string, onError:(message:string)=>void) {
  ...
  const img = new Image();
  img.onload = () => { const box=svg.viewBox.baseVal; const canvas = document.createElement('canvas'); canvas.width = box?.width || 900; canvas.height = box?.height || 560; const ctx = canvas.getContext('2d'); if (!ctx) { onError('Export PNG failed: canvas is unavailable.'); return; } ctx.fillStyle = '#fff'; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.drawImage(img,0,0,canvas.width,canvas.height); URL.revokeObjectURL(url); const a = document.createElement('a'); const safeTime = (timeH || 'unknown').replace(/[^0-9.]+/g, '_'); a.download = `TID_map_${prn || 'ALL'}_${safeTime}h.png`; a.href = canvas.toDataURL('image/png'); a.click(); onError(''); };
...
function PreviewPlot({raster,rasterOpacity,points,stationMarkers,world,solarGeometry,layers,selected,onStationClick,prn,actualTimeH,viewport,setViewport,enlarged,display=DEFAULT_MAP_DISPLAY}:any) { const [tip,setTip]=useState(''); const drag=useRef<any>(null); const v:Viewport=viewport || EUROPE_VIEWPORT; const stationByCode = new Map<string, StationMarker>(stationMarkers.map((m:StationMarker)=>[m.station, m])); const visiblePoints = points; const visibleStations = stationMarkers; const w=900,h=560,padL=55,padT=55,padB=55,plotW=690,plotH=h-padT-padB,cbX=790,clipId=`geoClip-${enlarged?'large':'main'}`; const x=(lon:number)=>padL+(lon-v.lon_min)/(v.lon_max-v.lon_min)*plotW; const y=(lat:number)=>padT+(v.lat_max-lat)/(v.lat_max-v.lat_min)*plotH; const geo=(e:any)=>{ const r=e.currentTarget.getBoundingClientRect(); const sx=(e.clientX-r.left)/r.width*w, sy=(e.clientY-r.top)/r.height*h; return { lon:v.lon_min+(sx-padL)/plotW*(v.lon_max-v.lon_min), lat:v.lat_max-(sy-padT)/plotH*(v.lat_max-v.lat_min) }; }; const zoom=(factor:number, lon=(v.lon_min+v.lon_max)/2, lat=(v.lat_min+v.lat_max)/2)=>setViewport?.((old:Viewport)=>zoomViewport(old,lon,lat,factor)); return <><div className="mapToolbar"><button onClick={()=>zoom(0.8)}>Zoom in</button><button onClick={()=>zoom(1.25)}>Zoom out</button><button onClick={()=>setViewport?.(EUROPE_VIEWPORT)}>Reset view</button><span>{v.lon_min.toFixed(1)}..{v.lon_max.toFixed(1)}° lon · {v.lat_min.toFixed(1)}..{v.lat_max.toFixed(1)}° lat</span></div><svg viewBox={`0 0 ${w} ${h}`} className={`plot ${enlarged?'plotLarge':''}`} data-lon-min={v.lon_min} data-lon-max={v.lon_max} data-lat-min={v.lat_min} data-lat-max={v.lat_max} onWheel={(e:any)=>{e.preventDefault(); const g=geo(e); zoom(e.deltaY<0?0.8:1.25,g.lon,g.lat);}} onDoubleClick={()=>setViewport?.(EUROPE_VIEWPORT)} onMouseDown={(e:any)=>{ if(e.button===0){ drag.current={x:e.clientX,y:e.clientY,v}; } }} onMouseMove={(e:any)=>{ if(!drag.current) return; const r=e.currentTarget.getBoundingClientRect(); const dx=(e.clientX-drag.current.x)/r.width*w, dy=(e.clientY-drag.current.y)/r.height*h; setViewport?.(panViewport(drag.current.v, -dx/plotW*(drag.current.v.lon_max-drag.current.v.lon_min), dy/plotH*(drag.current.v.lat_max-drag.current.v.lat_min))); }} onMouseUp={()=>drag.current=null} onMouseLeave={()=>drag.current=null}><defs><clipPath id={clipId}><rect x={padL} y={padT} width={plotW} height={plotH}/></clipPath></defs><rect x="0" y="0" width={w} height={h} fill="#fff"/><text x="55" y="28" className="mapTitle">{`PRN ${prn || 'All'}${actualTimeH ? ` · epoch ${actualTimeH} h` : ''}${solarGeometry?.utc_datetime ? ` · ${solarGeometry.utc_datetime}` : ''}`}</text><g className="geographicLayers" clipPath={`url(#${clipId})`}>{layers.solar && layers.solarShade && solarGeometry && <SolarShading solar={solarGeometry} x={x} y={y} viewport={v}/>} {layers.dtec && raster?.image_href && <image className="dtecRaster" href={raster.image_href} x={padL} y={padT} width={plotW} height={plotH} opacity={rasterOpacity/100} preserveAspectRatio="none"><title>{`Natural-neighbour grid ready: ${raster.prn}, ${Number(raster.actual_time_h).toFixed(5)} h, ${raster.station_count} stations`}</title></image>}{layers.borders && <Borders world={world} x={x} y={y} viewport={v}/>} {layers.grid && <g className="grid internalGrid">{mapGridValues(v.lon_min,v.lon_max).map(l=><line key={'x'+l} x1={x(l)} x2={x(l)} y1={padT} y2={padT+plotH}/>)}{mapGridValues(v.lat_min,v.lat_max).map(l=><line key={'y'+l} x1={padL} x2={padL+plotW} y1={y(l)} y2={y(l)}/>)}</g>}{layers.solar && solarGeometry && <SolarContours solar={solarGeometry} layers={layers} x={x} y={y} viewport={v}/>} {layers.rays && <g className="stationRays" style={{'--ray-width':display.rayWidth,'--ray-opacity':display.rayOpacity/100} as any}>{visiblePoints.map((p:Point,i:number)=>{ const m=stationByCode.get(p.station); if(!m) return null; return <line key={`ray-${i}`} x1={x(m.lon)} y1={y(m.lat)} x2={x(p.ipp_lon)} y2={y(p.ipp_lat)}/>;})}</g>} {layers.ipp && <g className="ippMarkers">{visiblePoints.map((p:Point,i:number)=><circle key={i} cx={x(p.ipp_lon)} cy={y(p.ipp_lat)} r={display.ippMarkerSize} fill={dtecColor(p.dtec)} stroke="#44515f" onMouseEnter={()=>setTip(`IPP ${p.station}|${p.prn}; dTEC=${p.dtec} TECU time=${p.time_h} h elev=${p.elevation}; lon=${p.ipp_lon.toFixed(4)} lat=${p.ipp_lat.toFixed(4)}`)}><title>{`IPP for station ${p.station}; PRN ${p.prn}; dTEC=${p.dtec} TECU; time=${p.time_h} h; elevation=${p.elevation}; IPP lon=${p.ipp_lon}, lat=${p.ipp_lat}`}</title></circle>)}</g>}{layers.stations && <g className="stationMarkers">{visibleStations.map((m:StationMarker)=>{ const ids = selected.filter((id:string)=>id.startsWith(`${m.station}|`)); const sel = ids.length > 0; const sx=x(m.lon), sy=y(m.lat); return <g key={m.station} className={`stationMarker ${sel?'selectedStation':''}`} onMouseEnter={()=>setTip(`${m.station} — ${m.city || 'Unknown city'}, ${m.country || 'unknown'}; ${m.lat.toFixed(4)}, ${m.lon.toFixed(4)}; ${m.full_site_id || 'no identifier'}`)}><title>{`${m.station} — ${m.city || 'Unknown city'}, ${m.country || 'unknown'}; lat ${m.lat.toFixed(4)}, lon ${m.lon.toFixed(4)}; id ${m.full_site_id || 'n/a'}.`}</title>{sel && <circle cx={sx} cy={sy} r={display.stationMarkerSize+3} className="stationHalo"/>}<path className="stationStar" d={fourPointStarPath(sx,sy,display.stationMarkerSize/2)}/>{layers.stationLabels && <text className="stationLabel" x={sx+display.stationMarkerSize*.7} y={sy-display.stationMarkerSize*.35} style={{fontSize:display.stationLabelSize}}>{m.station}</text>}<circle cx={sx} cy={sy} r={Math.max(8, display.stationMarkerSize)} className="stationHitTarget" onClick={()=>onStationClick(m.station, prn)}/></g>;})}</g>}</g><g className="ticks">{mapGridValues(v.lon_min,v.lon_max).map(l=><g key={'tx'+l}><line x1={x(l)} x2={x(l)} y1={padT+plotH} y2={padT+plotH+5}/><text x={x(l)-10} y={h-18}>{l}°</text></g>)}{mapGridValues(v.lat_min,v.lat_max).map(l=><g key={'ty'+l}><line x1={padL-5} x2={padL} y1={y(l)} y2={y(l)}/><text x={12} y={y(l)+4}>{l}°</text></g>)}</g><rect className="mapFrame" x={padL} y={padT} width={plotW} height={plotH} fill="none" stroke="#526070"/><Colorbar x={cbX} y={padT} height={plotH}/><metadata>{`viewport lon_min=${v.lon_min} lon_max=${v.lon_max} lat_min=${v.lat_min} lat_max=${v.lat_max}`}</metadata></svg><p className="tooltip">{tip || 'Click station markers to toggle STATION|PRN selection; IPP markers show hover details only.'}</p></>; }

function twilightFill(e:number) { if(e >= 0) return 'transparent'; if(e >= -6) return 'rgba(91,111,134,0.18)'; if(e >= -12) return 'rgba(61,80,108,0.28)'; if(e >= -18) return 'rgba(35,52,78,0.40)'; return 'rgba(13,27,50,0.52)'; }
function SolarShading({solar,x,y,viewport}:any) {
  const rects:any[]=[]; const lons:number[]=solar.lon_values, lats:number[]=solar.lat_values, vals:number[][]=solar.solar_elevation_deg;
  for(let iy=0; iy<lats.length-1; iy++) for(let ix=0; ix<lons.length-1; ix++) {
    const lon=lons[ix], lat=lats[iy]; if(lons[ix+1] < viewport.lon_min || lon > viewport.lon_max || lats[iy+1] < viewport.lat_min || lat > viewport.lat_max) continue;
    const e=(vals[iy][ix]+vals[iy+1][ix]+vals[iy][ix+1]+vals[iy+1][ix+1])/4; const fill=twilightFill(e); if(fill==='transparent') continue;
    const x0=x(lon), x1=x(lons[ix+1]), y0=y(lats[iy+1]), y1=y(lat); rects.push(<rect key={`${ix}-${iy}`} x={Math.min(x0,x1)} y={Math.min(y0,y1)} width={Math.abs(x1-x0)+0.6} height={Math.abs(y1-y0)+0.6} fill={fill}/>);
  }
  return <g className="twilightShading"><title>Twilight shading from solar elevation: daylight transparent, darker below 0°, -6°, -12°, and -18°.</title>{rects}</g>;
}
function contourSegments(solar:any, threshold:number, x:any, y:any, viewport:Viewport) {
  const segs:string[]=[]; const lons:number[]=solar.lon_values, lats:number[]=solar.lat_values, vals:number[][]=solar.solar_elevation_deg;
  const interp=(a:number,b:number,t:number)=> Math.abs(b-a)<1e-9 ? 0.5 : (t-a)/(b-a);
  for(let iy=0; iy<lats.length-1; iy++) for(let ix=0; ix<lons.length-1; ix++) {
    if(lons[ix+1] < viewport.lon_min || lons[ix] > viewport.lon_max || lats[iy+1] < viewport.lat_min || lats[iy] > viewport.lat_max) continue;
    const v00=vals[iy][ix], v10=vals[iy][ix+1], v11=vals[iy+1][ix+1], v01=vals[iy+1][ix]; const pts:[number,number][]=[];
    if((v00-threshold)*(v10-threshold)<=0 && v00!==v10) pts.push([lons[ix]+interp(v00,v10,threshold)*(lons[ix+1]-lons[ix]), lats[iy]]);
    if((v10-threshold)*(v11-threshold)<=0 && v10!==v11) pts.push([lons[ix+1], lats[iy]+interp(v10,v11,threshold)*(lats[iy+1]-lats[iy])]);
    if((v01-threshold)*(v11-threshold)<=0 && v01!==v11) pts.push([lons[ix]+interp(v01,v11,threshold)*(lons[ix+1]-lons[ix]), lats[iy+1]]);
    if((v00-threshold)*(v01-threshold)<=0 && v00!==v01) pts.push([lons[ix], lats[iy]+interp(v00,v01,threshold)*(lats[iy+1]-lats[iy])]);
    for(let p=0; p+1<pts.length; p+=2) segs.push(`M${x(pts[p][0])},${y(pts[p][1])}L${x(pts[p+1][0])},${y(pts[p+1][1])}`);
  }
  return segs;
}
