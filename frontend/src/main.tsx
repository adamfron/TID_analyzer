...
function Visibility({arcs,load,choose,loading,summary,progress,startInterpolation,cancelInterpolation}:any) {
  ...
  const statusText=(a:Arc)=>{ if(!a.eligible_for_interpolation) return 'Ineligible'; const s=(a.interpolation_status||'not_generated').replace('_',' '); if(s==='ready') return 'Ready'; if(s==='partial') return 'Partial'; if(s==='failed') return 'Failed'; return 'Not generated'; };
  const running = progress?.state === 'running';
  return <div><button onClick={load} disabled={loading} title="Load per-PRN visibility arcs from imported observations.">{loading ? 'Loading visibility…' : 'Load visibility'}</button><div className="buttonRow">{running ? <button onClick={cancelInterpolation}>Cancel interpolation</button> : <button onClick={()=>startInterpolation(false)}>Interpolate all eligible maps now</button>}</div><details><summary>Advanced</summary><button onClick={()=>startInterpolation(true)}>Rebuild interpolated maps</button></details>{running && <p className="mini">{progress.current_prn || 'PRN'} · arc {progress.current_arc_index ?? '—'} · epoch {progress.current_epoch_index ?? '—'}<br/>Generated {progress.generated} · skipped {progress.already_ready + progress.skipped} · failed {progress.failed}</p>}<p className="mini">{loading ? 'Loading visibility…' : `${arcs.length} visibility arcs loaded.`}</p><table><thead><tr><th>PRN</th><th>arc</th><th>start</th><th>end</th><th>min</th><th>rows</th><th>sta</th><th>epochs</th><th>maps</th><th>status</th></tr></thead><tbody>{arcs.map((a:Arc)=>{ const reasons = (a.ineligibility_reasons || []).join('; '); return <tr key={`${a.prn}-${a.arc_index}`} className={a.eligible_for_interpolation ? '' : 'ineligibleArc'} title={reasons || 'Eligible for interpolation'} onClick={()=>choose(a)}><td>{a.prn}</td><td>{a.arc_index}</td><td>{a.start_time_h.toFixed(2)}</td><td>{a.end_time_h.toFixed(2)}</td><td>{a.duration_min.toFixed(1)}</td><td>{a.row_count}</td><td>{a.station_count}</td><td>{a.epoch_count}</td><td>{a.generated_map_count || 0} / {a.epoch_count}</td><td className={a.eligible_for_interpolation ? '' : 'ineligibleStatus'}>{statusText(a)}</td></tr>; })}</tbody></table><div className="interpSummary"><p>Eligible arcs: {summary?.eligible_arc_count ?? eligible.length} / {(summary?.eligible_arc_count ?? eligible.length) + (summary?.ineligible_arc_count ?? (arcs.length-eligible.length))}</p><p>Maps planned: {plannedMaps}</p><p>Maps generated: {summary?.ready_map_count ?? 0}</p><p>Missing maps: {summary?.missing_map_count ?? Math.max(0, plannedMaps)}</p><p>Failed maps: {summary?.failed_map_count ?? 0}</p></div></div>;
}
